...
export default function ChatPage({ params }: { params: Promise<{ userId: string }> }) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-6rem)]">
      {/* Header */}
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 px-5 py-4 flex items-center gap-3 mb-4 shrink-0">
        <button onClick={() => router.back()} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 transition-colors">
        ...
        {otherUser && (
          <Link href={`/profile/${otherUser.id}`} className="flex items-center gap-3">
            ...
            <span className="font-semibold text-slate-900 dark:text-slate-100">{otherUser.username}</span>
          </Link>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-3 px-1 mb-4">
        {msgs.length === 0 && (
          <p className="text-center text-slate-400 py-8">No messages yet. Send the first one!</p>
        )}
        {msgs.map(m => {
          ...
          return (
            <div key={m.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'} gap-2`}>
              ...
              <div className={`max-w-xs xl:max-w-sm px-4 py-2.5 rounded-2xl ${
                isMine
      ...
      <form onSubmit={send} className="flex gap-3 shrink-0">
        <input
          ...
          className="flex-1 px-5 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl outline-none focus:border-blue-400 transition-colors text-slate-900 dark:text-slate-100"
        />
        <button
          type="submit"
          disabled={sending || !content.trim()}
          className="px-5 py-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-semibold rounded-2xl transition-colors"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
