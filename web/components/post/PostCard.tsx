      ...
      <article className="bg-white dark:bg-[#0a1628] rounded-2xl border border-blue-100 dark:border-[#162033] overflow-hidden hover:border-blue-300 dark:hover:border-blue-800/60 transition-all fade-in group">
        <div className="flex items-center justify-between px-5 pt-4 pb-3">
          <Link href={`/profile/${post.author.id}`} className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-slate-900 dark:text-slate-100 hover:text-blue-500 dark:hover:text-blue-400 transition-colors leading-none mb-0.5">
          ...
          <div className="flex items-center gap-1">
            {currentUserId === post.user_id && (
              <button
                ...
                disabled={deleting}
                className="p-2 rounded-xl opacity-0 group-hover:opacity-100 hover:bg-rose-50 dark:hover:bg-rose-900/20 text-slate-300 dark:text-slate-600 hover:text-rose-500 transition-all"
              >
        ...
        {post.content && (
          <Link href={`/post/${post.id}`}>
            <p className={`px-5 ${post.image_url ? 'pb-3' : 'pb-4'} text-slate-800 dark:text-slate-200 leading-relaxed whitespace-pre-wrap text-[15px] ${compact ? 'line-clamp-4' : ''}`}>
              {post.content}
            </p>
          </Link>
        )}
        ...
        <div className="flex items-center justify-between px-3 py-2 border-t border-blue-50 dark:border-[#162033]">
          <div className="flex items-center gap-0.5">
            ...
            <Link
              href={`/post/${post.id}`}
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium text-slate-400 dark:text-slate-500 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/10 transition-all"
            >
            ...
            <button
              onClick={handleShare}
              className="flex items-center px-3 py-2 rounded-xl text-slate-400 dark:text-slate-500 hover:text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/10 transition-all"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
